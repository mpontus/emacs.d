...
Aspell, which is selected by default, appears to fail sometimes during ~flyspell-buffer~.
...
(defhydra origami-hydra (:hint nil)
  "
     ^Basic^               ^All^            ^Movement^           ^History^ 
------------------- ------------------- ------------------- ------------------- 
[_o_] Open          [_O_] Open All      [_p_] Previous      [_C-/_] Undo
[_c_] Close         [_C_] Close All     [_n_] Next          [_C-S-/_] Redo
[_t_] Toggle        [_T_] Toggle All    [_ _] Forward       [_r_] Reset
[_s_] Show     	    [_S_] Show Only     [_f_] Forward Same  ^ ^
^ ^                 ^ ^                 [_b_] Backward Same ^ ^
"
  ("o" origami-open-node :exit t)
  ("c" origami-close-node :exit t)
  ("t" origami-toggle-node :exit t)
  ("s" origami-show-node :exit t)
  ...
  ;; All
  ("O" origami-open-all-nodes :exit t)
  ("C" origami-close-all-nodes :exit t)
  ("T" origami-toggle-all-nodes :exit t)
  ("S" origami-show-only-node :exit t)
  ...
  ("C-/" origami-undo)
  ("C-S-/" origami-redo)
  ("r" origami-reset :exit t))

(add-hook 'typescript-mode-hook 'origami-mode)

(define-key typescript-mode-map (kbd "C-c C-f") 'origami-toggle-node)
(define-key typescript-mode-map (kbd "C-c f") 'origami-hydra/body)
#+END_SRC
