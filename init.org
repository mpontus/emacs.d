...
#+begin_src emacs-lisp
(defun init-org-validate-syntax ()
  (interactive)
  (when (file-equal-p (or (buffer-file-name) "")
	  ...
	  (with-syntax-table emacs-lisp-mode-syntax-table
	    (while (not (eobp))
	      (and (zerop (skip-chars-forward "\s\t\r\n"))
		   (null (comment-forward 1))
		   (read (current-buffer))))))))))

(add-hook 'before-save-hook #'init-org-validate-syntax)
#+end_src
...
#+begin_src emacs-lisp
(use-package ivy :bind ("C-c C-c" . ivy-restrict-to-matches))
#+end_src
