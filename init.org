# -*- org-content-indentation: 0; org-src-preserve-indentation: t; -*-
...
(use-package ivy
  ...
  (define-key ivy-minibuffer-map (kbd "C-M-s")
    #'ivy-restrict-to-matches))
...
;;    'counsel-M-x
;;    `((,(-compose 'file-name-base 'symbol-file 'intern)
;;       (:face file-name-shadow :width 20)
;;       ()))))
#+end_src
...
(use-package help-macro+
  :quelpa (help-macro+ :fetcher wiki)) 
#+end_src
...
Insert template upon opening a new file:
...
(use-package cell-mode
  :quelpa
  (cell-mode :fetcher git :url "https://gitlab.com/dto/cell-mode.git" :commit "52cd43318712a9afc0accefdd61bf9228ba36b58"))
#+end_src
* Git
...
(setq display-buffer-alist
      '(("^\\*compilation\\*$" (display-buffer-reuse-window)) ("`\\*Warnings\\*$" display-buffer-other-window)))
(add-to-list 'display-buffer-alist
	     '(("`\\*git-gutter:diff\\*$" (display-buffer-same-window))))
...
Insert template into new ~.nix~ files.

#+begin_src emacs-lisp
(use-package autoinsert
  :config
  (add-to-list 'auto-insert-alist (cons 'nix-mode "nixpkg.nix")))
#+end_src
